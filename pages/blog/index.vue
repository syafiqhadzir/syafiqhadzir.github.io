<script lang="ts" setup></script>

<template>
  <main class="min-h-screen bg-slate-200 text-slate-700 dark:bg-black dark:text-slate-400">
    <div class="grid gap-10 place-content-center p-8 md:grid-cols-2 lg:w-2/3 xl:grid-cols-3 xl:w-2/3 mx-auto">
      <!--
        Generates a card for each article contained in the 'blog' subfolder
        (./content/blog) and displays front-matter data (title, date...)
      -->
      <ContentList v-slot="{ list }" path="/blog">
        <div
          v-for="article in list"
          :key="article._path"
          class="bg-slate-100 dark:bg-neutral-900 h-fit px-4 pt-4 pb-6 rounded-lg border border-transparent hover:border-blue-500 duration-200"
        >
          <h2 class="text-2xl font-bold tracking-wide w-fit">
            {{ article.title }}
          </h2>
          <div class="flex flex-row items-center">
            <p>{{ article.author }}</p>
            <p class="mx-4">
              {{ article.date }}
            </p>
          </div>
          <p class="mt-4 mb-8">
            {{ article.description }}
          </p>
          <NuxtLink
            :to="article._path"
            class="bg-slate-200 dark:bg-black hover:bg-blue-500 hover:text-white duration-200 px-5 py-2 rounded-full"
          >
            Read more
          </NuxtLink>
        </div>
      </ContentList>
    </div>
  </main>
</template>

<style scoped>

</style>
